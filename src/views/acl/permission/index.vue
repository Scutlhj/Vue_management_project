...
import { reqAddOrUpdatePermission, reqDeletePermission, reqGetAllPermission } from '@/api/acl/menu';
...
const rules = {
  name: [
    ...
    {
      min: 2, max: 20, message: '名称长度需为2到20位', trigger: 'blur'
    },
  ],
  code: [
    {
      required: true, trigger: 'blur', message: '权限值不能为空'
    },
    {
  ...
  <div>
    <el-table :data="permissions" border row-key="id">
      ...
      <el-table-column width="250px" label="操作" align="center" :resizable="false">
        <template #="{ row, $index }">
          <el-button size="small" type="success" :disabled="row.level === 4" @click="addPermission(row)">{{ row.level ===
            3 ? '添加功能' : '添加菜单' }}</el-button>
          <el-button size="small" type="primary" :disabled="row.level === 1" @click="editPermission(row)">编辑</el-button>
    ...
    <el-dialog v-model="dialogFormVisible"
      :title="permissionParams.id ? `编辑${permissionParams.level === 4 ? '功能' : '菜单'}` : `添加${permissionParams.level === 4 ? '功能' : '菜单'}`"
      width="35%">
