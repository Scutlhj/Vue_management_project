...
import type { Records, TrademarkResponseData, TrademarkData } from '@/api/product/trademark/type'
...
const confirm = async () => {
  ...
  if (result.code == 200) {
    ...
    ElMessage.success(`${trademarkFormData.id ? '修改品牌成功' : '添加品牌成功'}`)
    // 修改的话留在当前页面
...
<template>
  <div>
    <el-card class="box-card">
      ...
      <el-table border :data="trademarkArr" style="margin-bottom: 20px;">
        ...
        <el-table-column label="品牌名称" align="center">
          ...
          <template #="{ row, column, $index }">
            <pre style="color:black;font-size: 20px; font-weight: bold;">{{ row.tmName }}</pre>
          </template>
        </el-table-column>
        ...
        <el-table-column label="品牌操作" align="center">
          <template #="{ row, column, $index }">
            ...
            <el-popconfirm width="150px" confirm-button-text="确定" cancel-button-text="取消" icon="Delete"
              icon-color="#626AEF" :title="`确认删除${row.tmName}吗?`" @confirm="deleteTrademark(row.id)">
              <template #reference>
      ...
      <!-- 分页器组件 -->
      <!-- layout表示六个子组件布局方式 -->
      <el-pagination v-model:currentPage="pageNo" :page-sizes="[3, 5, 7, 9]" v-model:page-size="limit"
    ...
    <el-dialog v-model="dialogFormVisible" :title="trademarkFormData.id ? '修改品牌' : '添加品牌'">
      <el-form :model="trademarkFormData" style="width: 80%;" :rules="checkrules" ref="TrademarkForm">
        ...
        <el-form-item label="品牌Logo" label-width="100px" prop="logoUrl">
          ...
          <el-upload class="avatar-uploader" action="/api/admin/product/fileUpload" :show-file-list="false"
            ...
            <img v-if="trademarkFormData.logoUrl" :src="trademarkFormData.logoUrl" class="avatar" />
