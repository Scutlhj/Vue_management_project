...
const getAttrList = async () => {
  let result: AttrResponseData = await reqGetAttr(
    categoryStore.c1SelectedId as number,
...
const toShow = (row: AttrValue, index: number) => {
  ...
  // 非法情况二:输入有冲突
  if (
    attrParams.attrValueList.find(
      (item) => item !== row && item.valueName === row.valueName.trim(),
    )
  ) {
        ...
        >
          ...
          <el-table-column label="属性值" :resizable="false">
            <template #="{ row, column, $index }">
              ...
              ></el-input>
              <el-tag v-if="row.show" @click="toEdit($index)" size="large">
                {{ row.valueName }}
              </el-tag>
            </template>
          </el-table-column>
