...
import type { SpuResponseData, SpuRecords, SpuData, SkuData } from '@/api/product/spu/type'
...
const getSpuList = async (pager = 1) => {
  // 修改页码
  pageNo.value = pager
  let result: SpuResponseData = await reqGetSpu(pageNo.value, pageSize.value, categoryStore.c3SelectedId)
  if (result.code === 200) {
...
const addSku = (row: SpuData) => {
  skuform.value.initAddsku(categoryStore.c1SelectedId, categoryStore.c2SelectedId, row)
  scene.value = 2
}
    ...
    <el-card shadow="always" :body-style="{ padding: '20px' }" style="margin: 20px 0">
      ...
      <div v-show="scene === 0">
        ...
        <el-table border style="margin: 20px 0" :data="spuRecords">
          <el-table-column type="index" label="序号" width="100px" align="center" :resizable="false" />
          <el-table-column label="SPU名称" prop="spuName" width="150px" :resizable="false"></el-table-column>
          <el-table-column label="SPU描述" prop="description" show-overflow-tooltip :resizable="false"></el-table-column>
          <el-table-column label="操作" :resizable="false" width="250px">
            <template #="{ row, $index }">
              ...
              <el-button type="warning" size="default" icon="Edit" title="修改SPU" @click="editSpu(row)"></el-button>
              <el-button type="info" size="default" icon="View" title="查看SKU列表" @click="getSkuList(row)"></el-button>
              <el-popconfirm width="220" confirm-button-text="确定" cancel-button-text="取消" icon="Delete"
                icon-color="#626AEF" :title="`你确认删除${row.spuName}吗?`" @confirm="deleteSpu(row)">
                <template #reference>
        ...
        <el-pagination v-model:currentPage="pageNo" :page-sizes="[3, 5, 7, 9]" v-model:page-size="pageSize"
          layout="prev, pager, next, jumper,->,sizes,total" :total="total" :background="true" :pager-count="9"
          @current-change="getSpuList" @size-change="getSpuList()"></el-pagination>
      </div>
      ...
      <el-dialog v-model="showDialog" title="SKU展示列表" width="920px">
        <el-table :data="skuList" border>
          <el-table-column label="序号" type="index" width="80px" align="center" :resizable="false" />
          <el-table-column label="SKU名字" prop="skuName" width="300px" align="center" :resizable="false" />
          <el-table-column label="SKU价格" prop="price" width="150px" align="center" :resizable="false" />
          <el-table-column label="SKU重量" prop="weight" width="150px" align="center" :resizable="false" />
          <el-table-column label="SKU图片" width="200px" align="center" :resizable="false">
            <template #="{ row, $index }">
