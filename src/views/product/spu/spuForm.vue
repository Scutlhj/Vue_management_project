...
import { reqGetTrademarks, reqGetSpuImage, reqGetSaleAttr, reqGetAllSaleAttr, reqPostAddOrUpdateSpu } from '@/api/product/spu'
import type { SaleAttrObj, SpuSaleAttr, SpuImage, Trademark, SpuData, TrademarkResponseData, SpuImageResponseData, SpuSaleAttrsResponseData, SaleAttrResponseData, SpuSaleAttrValue } from '@/api/product/spu/type'
...
const getSpuInfo = async (row: SpuData) => {
    ...
    if (result1.code === 200 && result2.code === 200 && result3.code === 200 && result4.code === 200) {
...
const toShow = (row: SpuSaleAttr) => {
    ...
    if (!row.spuSaleAttrValueList.every((item) => saleAttrValueName.trim() !== item.saleAttrValueName)) {
...
<template>
    <el-form label-width="80px" :rules="checkrules" ref="spuForm" :model="SpuParams">
        <el-form-item label="SPU名称" size="default" prop="spuName">
            <el-input placeholder="请输入SPU名称" size="default" v-model="SpuParams.spuName"></el-input>
        </el-form-item>
        <el-form-item label="SPU品牌" prop="tmId">
            <el-select v-model="SpuParams.tmId">
                <el-option v-for="(item, index) in TrademarkArr" :key="item.id" :label="item.tmName"
                    :value="item.id"></el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="SPU描述" size="default">
            <el-input type="textarea" placeholder="请输入SPU描述" size="default" v-model="SpuParams.description"></el-input>
        </el-form-item>
        <el-form-item label="SPU图片" size="default">
            ...
            <el-upload v-model:file-list="ImageArr" action="/api/admin/product/fileUpload" list-type="picture-card"
        ...
        <el-form-item label="SPU属性" size="default">
            <el-select v-model="selectedAttr"
                :placeholder="unselectAttr.length > 0 ? `还有${unselectAttr.length}个未选择属性` : '没有可选择的属性'">
                <el-option v-for="(item, index) in unselectAttr" :key="item.id" :label="item.name"
                    :value="`${item.id}:${item.name}`"></el-option>
            </el-select>
            <el-button type="primary" size="default" icon="Plus" style="margin-left: 18px;"
                :disabled="selectedAttr ? false : true" @click="addAttr">添加属性</el-button>
            <el-table border style="margin-top: 18px;" :data="spuSaleAttr">
                <el-table-column type="index" label="序号" width="80px" align="center" :resizable="false" />
                <el-table-column label="属性名" width="150px" :resizable="false" prop="saleAttrName"></el-table-column>
                <el-table-column label="属性值" :resizable="false">
                    <template #="{ row, $index }">
                        ...
                        <el-input placeholder="请输入属性值" size="small" clearable @change="" style="width: 100px;"
                            v-if="row.flag" @blur="toShow(row)" v-model="row.spuSaleAttrValue"
                            ref="attrValueInput"></el-input>
                        <el-button type="success" size="small" icon="Plus" @click="toEdit(row)" v-else></el-button>
                    </template>
                </el-table-column>
                <el-table-column label="操作" width="80px" :resizable="false">
                    <template #="{ row, $index }">
                        <el-button type="danger" size="default" @click="spuSaleAttr.splice($index, 1)"
        ...
        <el-form-item size="default">
            <el-button type="primary" size="default" @click="saveAttr"
                :disabled="spuSaleAttr.length > 0 ? false : true">保存</el-button>
            <el-button size="default"
                @click="$emit('changeScene', { num: 0, action: SpuParams.id ? 'update' : 'add' })">取消</el-button>
        </el-form-item>
